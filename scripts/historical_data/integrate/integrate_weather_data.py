...
def load_weather_data(conn):
    ...
    try:
        ...
        # Standardize location names to match live fetching format
        ...
        for col in ['snowfall', 'humidity', 'cloud_coverage']:
            if col not in df:
                df[col] = 0
        
        # Keep only the required columns
        required_columns = ['date', 'location', 'avg_temperature', 'precipitation', 'wind_speed', 'snowfall', 'humidity', 'cloud_coverage']
...
def store_weather_data(df, conn):
    ...
    for _, row in df.iterrows():
        ...
        print(f"Inserting weather data for {location} at {timestamp}")
        logging.info(f"Inserting weather data for {location} at {timestamp}")
        
        try:
            cur.execute("""
                INSERT INTO weather_data (date, location, avg_temperature, precipitation, wind_speed, snowfall, humidity, cloud_coverage)
            ...
            print(f"Error inserting weather data for {location} at {timestamp}: {e}")
            logging.error(f"Error inserting weather data for {location} at {timestamp}: {e}")
            conn.rollback()
